...
| **Student** | Google OAuth | Organization (all tiers) | Self-registration via Google Sign-In (public link, any tier) or Premium student portal → org verification | Supabase session cookie (httpOnly) |
...
### Student Authentication (All Tiers — Portal Access Premium Only)

Students authenticate via **Google OAuth**. OAuth is available to **all tiers** — students from any organization can authenticate via a public registration link generated by an officer (see [`org_registration_links`](./05-database-schema.md#49-org_registration_links)). However, **authentication does not imply dashboard access**. Full student portal access (dashboard, self-service payments, clearance, appeals) is gated to the **Premium tier** and requires `is_verified = true`.

- **Basic/Plus tiers:** Students authenticate via Google OAuth through a public registration link for **identity verification only**. After completing registration, they see a confirmation page — not a dashboard. Their `auth_user_id` is linked in the `students` table, and a `student_org_memberships` row is created.
- **Premium tier:** Students authenticate via Google OAuth and, once verified by an officer, gain full portal dashboard access.
- **Import-only students:** Students added via bulk CSV import have no auth account. They exist only in the `students` table with no `auth_user_id`.

#### Student `app_metadata` Structure

```jsonc
{
  "organization_id": "uuid",       // The org the student registered with
  "role": "student",
  "is_verified": false | true       // Set to true by an officer after org verification
}
```

> **Note:** `app_metadata` is set for all students who authenticate via Google OAuth, regardless of tier. Portal dashboard access is enforced at the **routing layer** and via **RLS policies** — not by blocking OAuth itself.

#### Student Registration Flow (Public Link — All Tiers)

```
1. Officer generates a public registration link for their organization
   (via org_registration_links table — available on all tiers)
2. Student visits the link and clicks "Sign in with Google"
3. Supabase Auth handles Google OAuth redirect
4. On callback, a database trigger / Server Action:
   a. Identifies the organization from the registration link context
   b. Sets app_metadata: { organization_id, role: "student", is_verified: false }
   c. Creates or links the student record in the `students` table
   d. Creates a `student_org_memberships` row:
      - status: `pending_approval` (if link requires approval) or `active` (if auto-approved)
5. Routing logic checks the organization's tier:
   a. Premium tier → Student lands on /portal/onboarding (self-registration form)
      - Student submits personal data (name, ID number, course, etc.)
      - Officer reviews and verifies the student record
      - On verification, a Server Action updates app_metadata: is_verified = true
        (via supabase.auth.admin.updateUserById())
      - Student now has full portal dashboard access
   b. Basic/Plus tier → Student lands on a confirmation page
      - Page confirms successful registration and org membership
      - No dashboard access — authentication was for identity verification only
```

#### Student Registration Flow (Premium Portal — Direct Login)

```
1. Verified Premium student visits /portal and clicks "Sign in with Google"
2. Supabase Auth handles Google OAuth redirect
3. On callback, the student's existing app_metadata is read:
   - organization_id and is_verified are already set from initial registration
4. Routing middleware checks:
   a. Organization tier = premium → allow portal access
   b. is_verified = true → allow full dashboard
   c. is_verified = false → redirect to pending verification page
   d. Organization tier != premium → redirect to error page
      ("Your organization's plan does not include the student portal")
```
...
> **Key distinction:** The OAuth callback does **not** block non-Premium organizations. The block happens at the **routing layer** for dashboard access only. This aligns with the Shared Student Data Model in [05-database-schema.md](./05-database-schema.md#2-shared-student-data-model), where students from any tier can authenticate via public links for identity verification.
  ...
  └── Admin (org-level — full org access)
       └── Manager (operational access — no role/subscription management)
            └── Staff (attendance + member directory — read-heavy, minimal writes)
                 └── Student (self-service — own data only, portal access Premium tier; OAuth registration all tiers)
```
